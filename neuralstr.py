...
def update_display():
    ...
    with st.sidebar:
        ...
        st.metric("Learning Rate", summary["learning_rate"])
        
        # Performance metrics
        st.subheader("Performance")
        st.text(f"Runtime: {summary['runtime']}")
        st.text(f"Steps: {summary['simulation_steps']}")
        st.text(f"Avg. Connections: {summary['avg_connections']}")
